...
void laplacian_s(matrix *fin,matrix *fout)
{
    ...
    for(i=0;i<=Nx+1;i++)
    {
        fin_all->data[i][Ny+1]=fin_all->data[i][Ny];
        fin_all->data[i][0]=fin_all->data[i][1];
    }
...
void doublelaplacian_s(matrix *fin,matrix *fout)
{
    ...
    {
        //y方向边界phi的法向导数为0
        fin_all->data[i][Ny+2]=fin_all->data[i][Ny+1];
        fin_all->data[i][1]=fin_all->data[i][2];
        
        //y方向边界拉普拉斯phi的法向导数为0
        fin_all->data[i][Ny+3]=fin_all->data[i][Ny];
        fin_all->data[i][0]=fin_all->data[i][3];
    }
    //利用差分表达二次拉普拉斯
    #pragma omp parallel for num_threads(8)
    for(i=0;i<Nx;i++)
...
void initialization(void)
{
    ...
    for(k=1;k<=3;k++)
    {
        ...
        for(i=0;i<=Nx-1;i++)
            for(j=2;j<=Ny-3;j++)
            {
                phi_s->data[i][j]=1.105682211*
                (0.110865*(Gaussian_blur_s->data[i][j-2]+Gaussian_blur_s->data[i][j+2])
                +0.210786*(Gaussian_blur_s->data[i][j-1]+Gaussian_blur_s->data[i][j+1])
                +0.261117*Gaussian_blur_s->data[i][j]);
                phi_p->data[i][j]=1.105682211*
                (0.110865*(Gaussian_blur_p->data[i][j-2]+Gaussian_blur_p->data[i][j+2])
                +0.210786*(Gaussian_blur_p->data[i][j-1]+Gaussian_blur_p->data[i][j+1])
                +0.261117*Gaussian_blur_p->data[i][j]);
            }
    }
    //加扰动
    double eps=1e-3;
